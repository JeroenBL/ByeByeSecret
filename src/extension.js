...
vscode.commands.registerCommand('byebyesecret.anonymizeSecrets', () => {
    ...
    editor.edit((editBuilder) => {
        editBuilder.replace(new vscode.Range(editor.document.positionAt(0), editor.document.positionAt(text.length)), anonymizedText);
    });
});
...
async function checkForUpdates() {
    if (extensionActivated) {
        ...
        if (latestVersion && latestVersion.version !== packageJson.version) {
            const downloadButton = { title: "Download Now" };
            const result = await vscode.window.showInformationMessage(`Version: [${latestVersion.version}] of ByeByeSecret is now available. Go to Github to download the latest version.`, downloadButton);
...
function anonymizeSecrets(text) {
